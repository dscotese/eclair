...
class NodeRelayerSpec extends TestkitBaseClass {
  ...
  test("fail to relay when expiry is too soon (single-part)") { f =>
    ...
    commandBuffer.expectMsg(CommandBuffer.CommandSend(p.add.channelId, p.add.id, CMD_FAIL_HTLC(p.add.id, Right(TrampolineExpiryTooSoon), commit = true)))
    commandBuffer.expectNoMsg(100 millis)
    outgoingPayFSM.expectNoMsg(100 millis)
  }
  ...
  test("fail to relay when expiry is too soon (multi-part)") { f =>
    ...
    p.foreach(p => commandBuffer.expectMsg(CommandBuffer.CommandSend(p.add.channelId, p.add.id, CMD_FAIL_HTLC(p.add.id, Right(TrampolineExpiryTooSoon), commit = true))))
    commandBuffer.expectNoMsg(100 millis)
    outgoingPayFSM.expectNoMsg(100 millis)
  }
  ...
  test("fail to relay when fees are insufficient (single-part)") { f =>
    ...
    commandBuffer.expectMsg(CommandBuffer.CommandSend(p.add.channelId, p.add.id, CMD_FAIL_HTLC(p.add.id, Right(TrampolineFeeInsufficient), commit = true)))
    commandBuffer.expectNoMsg(100 millis)
    outgoingPayFSM.expectNoMsg(100 millis)
  }
  ...
  test("fail to relay when fees are insufficient (multi-part)") { f =>
    ...
    p.foreach(p => commandBuffer.expectMsg(CommandBuffer.CommandSend(p.add.channelId, p.add.id, CMD_FAIL_HTLC(p.add.id, Right(TrampolineFeeInsufficient), commit = true))))
    commandBuffer.expectNoMsg(100 millis)
    outgoingPayFSM.expectNoMsg(100 millis)
  }
  ...
  test("fail to relay because outgoing balance isn't sufficient") { f =>
    ...
    incomingMultiPart.foreach(p => commandBuffer.expectMsg(CommandBuffer.CommandSend(p.add.channelId, p.add.id, CMD_FAIL_HTLC(p.add.id, Right(TemporaryNodeFailure), commit = true))))
    commandBuffer.expectNoMsg(100 millis)
  ...
  test("fail to relay because incoming fee isn't enough to find routes downstream") { f =>
    ...
    incomingMultiPart.foreach(p => commandBuffer.expectMsg(CommandBuffer.CommandSend(p.add.channelId, p.add.id, CMD_FAIL_HTLC(p.add.id, Right(TrampolineFeeInsufficient), commit = true))))
    commandBuffer.expectNoMsg(100 millis)
    eventListener.expectNoMsg(100 millis)
  }
  ...
  test("fail to relay because of downstream failures") { f =>
    ...
    outgoingPayFSM.send(nodeRelayer, PaymentFailed(outgoingPaymentId, paymentHash, failures))
    incomingMultiPart.foreach(p => commandBuffer.expectMsg(CommandBuffer.CommandSend(p.add.channelId, p.add.id, CMD_FAIL_HTLC(p.add.id, Right(FinalIncorrectHtlcAmount(42 msat)), commit = true))))
    commandBuffer.expectNoMsg(100 millis)
    eventListener.expectNoMsg(100 millis)
  }
